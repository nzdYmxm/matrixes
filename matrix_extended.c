...
void rows_maximums(int **matrix, int width, int height);
void cols_minimums(int **matrix, int width, int height);
    ...
    if (error) {
        ...
        if (error) {
            ...
            if (error) {
                ...
                printf("\n");
                rows_maximums(matrix, width, height);
                printf("\n");
                cols_minimums(matrix, width, height);
            }
        }
    }
...
bool get_matrix_data(int *mem_type, int *width, int *height) {
    ...
    if (has_scan_error(scan_result, tmp) || (*mem_type) < 0 || (*mem_type) > 3) {
        error = true;
    } else {
